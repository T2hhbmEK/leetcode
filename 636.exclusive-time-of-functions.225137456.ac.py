...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            job, action, timestamp = log.split(":")
            job = int(job)
            timestamp = int(timestamp)
            if action == "start":
                stack.append((job, timestamp, t))
                t = 0
            else:
                job_, timestamp_, t_ = stack.pop()
                # assert job_ == job
                total_time = timestamp - timestamp_ + 1
                exclusive_time = total_time - t
                r[job] += exclusive_time
                t = t_
                t += total_time
            # print("---")
